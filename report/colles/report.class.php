...
class report_colles extends mod_survey_reportbase {
    /*
     * template
     */
    public $template;
    ...
    function setup($hassubmissions, $group, $area, $qid) {
        ...
        $this->template = $DB->get_field('survey', 'template', array('id' => $this->survey->id));
    ...
    public function output_html($nexturl, $graphurl, $altkey) {
        ...
        if (empty($strseemoredetail)) {     // Cache the string for the next future
            $strseemoredetail = get_string('seemoredetail', 'surveyreport_colles');
        }
        ...
        $imgparams['alt'] = get_string($altkey, 'surveyreport_colles');
    ...
    public function get_qid_per_area() {
        ...
        $whereparams = array('surveyid' => $this->survey->id, 'plugin' => 'radiobutton');
    ...
    public function output_summarydata() {
        ...
        $graphurl = new moodle_url('/mod/survey/report/colles/graph.php', $paramurl);
    ...
    public function fetch_summarydata() {
        ...
        $this->graphtitle = get_string('summary', 'surveyreport_colles');

        // names of areas of investigation
        for ($i = 1; $i < 7; $i++) {
            $this->xlabels[] = get_string('fieldset_content_0'.$i, 'surveytemplate_'.$this->template);
        }
        ...
        $itemid = $qid1area[0][0]; // one of the itemid of the survey (the first)
        $item = survey_get_item($itemid, SURVEY_TYPEFIELD, 'radiobutton');
        $this->ylabels = $item->item_get_labels_array('options');
        ...
        if (!$this->canaccessreports && $this->canaccessownreports) { // if the user hasn't general right but only canaccessownreports
            $sqlparams = array('userid' => $USER->id);

            foreach ($toevaluate as $k => $qidarea) {
                foreach ($qidarea as $areaidlist) {
                    $sql = 'SELECT count(ud.id) as countofanswers, SUM(ud.content) as sumofanswers
                            FROM {survey_userdata} ud
                                JOIN {survey_submission} ss ON ss.id = ud.submissionid
                            WHERE ud.itemid IN ('.implode(',', $areaidlist).')
                            AND ss.userid = :userid';
                    $aggregate = $DB->get_record_sql($sql, $sqlparams);
    ...
    public function fetch_scalesdata($area=false) {
        ...
        if ($area === false) { // $area MUST BE provided
            debugging('Error at line '.__LINE__.' of '.__FILE__.'. Unexpected null $area');
        }

        $this->graphtitle = get_string('fieldset_content_0'.($area + 1), 'surveytemplate_'.$this->template);
        ...
        for ($i = 1; $i < 5; $i++) {
            ...
            $this->xlabels[] = get_string($key, 'surveyreport_colles');
        }
        // end of: names of areas of investigation
        ...
        foreach ($toevaluate as $k => $areaidlist) {
            foreach ($areaidlist as $itemid) {
                $sqlparam = array('itemid' => $itemid);

                $sql = 'SELECT count(ud.id) as countofanswers, SUM(ud.content) as sumofanswers
                        FROM {survey_userdata} ud
                        WHERE ud.itemid = :itemid';
                $aggregate = $DB->get_record_sql($sql, $sqlparam);
                $m = $aggregate->sumofanswers/$aggregate->countofanswers;
                if ($k == 0) {
                    $this->trend1[] = $m;
                }
                ...
                $sql = 'SELECT ud.content
                        FROM {survey_userdata} ud
                        WHERE ud.itemid = :itemid';
                $answers = $DB->get_recordset_sql($sql, $sqlparam);
                $bigsum = 0;
                foreach ($answers as $answer) {
                    $xi = (double)$answer->content;
                    $bigsum += ($xi - $m) * ($xi - $m);
    ...
    public function fetch_questionsdata($area=false, $qid=false) {
        ...
        if ($qid === false) { // $area MUST BE provided
            debugging('Error at line '.__LINE__.' of '.__FILE__.'. Unexpected null $qid');
        }
        ...
        // options (label of answers)
        $itemid = $qid1area[$area][$qid]; // one of the itemid of the survey (the first)
        $item = survey_get_item($itemid, SURVEY_TYPEFIELD, 'radiobutton');
        $this->xlabels = $item->item_get_labels_array('options');
        // end of: options (label of answers)
        ...
        foreach ($toevaluate as $k => $areaidlist) {
            $sql = 'SELECT content, count(id) as absolute
                    ...
                    WHERE itemid = :itemid
                    GROUP BY content';
            $sqlparam = array('itemid' => $areaidlist[$qid]);
            $aggregates = $DB->get_records_sql($sql, $sqlparam);
