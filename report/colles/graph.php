...
$url = new moodle_url('/mod/survey/report/frequency/graph.php', array('id' => $id));
if ($group !== 0) {
...
if ($type == 'summary') {
    ...
    if ($reportman->template == 'collesactualpreferred') {
        $legendgraph1 = get_string('collespreferred', 'surveyreport_colles');
        $legendgraph2 = get_string('collesactual', 'surveyreport_colles');
    } else {
        $legendgraph1 = get_string($reportman->template, 'surveyreport_colles');
    }
    ...
    if ($allowsingle) { // if the user hasn't general right but only canaccessownreports
        if ($reportman->studenttrend1) { // if the user submitted at least one response
            ...
            $graph1_params['legend'] = fullname($USER).': '.$legendgraph1;
            ...
            if ($reportman->template == 'collesactualpreferred') {
                ...
                $graph1_params['legend'] = fullname($USER).': '.$legendgraph2;
