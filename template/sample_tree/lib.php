...
function surveytemplate_sample_tree_add() {
    ...
    $content_sid = 0;
    // ////////////// SURVEY_ITEM
    $si_fields = array('surveyid','type','plugin','externalname','content_sid','content','contentformat','customnumber','extrarow','softinfo','required','fieldname','indent','basicform','advancedsearch','draft','sortindex','basicformpage','advancedformpage','parentid','parentcontent','parentvalue','timecreated','timemodified');
    ...
    $defaultvalue_sid = 0;
    // ////////////// SURVEY_NUMERIC
    $numeric_fields = array('surveyid','itemid','defaultvalue_sid','defaultvalue','signed','lowerbound','upperbound','decimals');
...
//----------------------------------------------------------------------------//
    ...
    /*------------------------------------------------*/
    $content_sid++;
    $values = array(0,SURVEY_FIELD,'numeric','sample_tree',
                    $content_sid,null,FORMAT_HTML,null,
                    0,'',SURVEY_OPTIONALITEM,null,
                    0,SURVEY_FILLONLY,SURVEY_ADVFILLONLY,0,
                    $sortindex,'','',1,
    ...
    $itemid = $DB->insert_record('survey_item', array_combine($si_fields, $values));
        ...
        $values = array(0,$itemid,$defaultvalue_sid,null,
                        '','','','');
        $itemid = $DB->insert_record('survey_numeric', array_combine($numeric_fields, $values));
    //---------- end of this item
