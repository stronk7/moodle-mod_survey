...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
    <xs:element name="surveyfield_shortdate">
        <xs:complexType>
            <xs:sequence>
...
EOS;
    ...
    public function userform_mform_validation($data, &$errors, $survey, $searchform) {
        ...
        if (!$searchform) {
            ...
            // but not only 1
            $testpassed = true;
            if ($data[$this->itemname.'_month'] == SURVEY_IGNOREME) {
        ...
        if (!$testpassed) {
            if ($this->required) {
                $errors[$errorkey] = get_string('uerr_shortdatenotsetrequired', 'surveyfield_shortdate');
            } else {
