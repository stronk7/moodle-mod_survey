...
class surveyfield_age extends surveyitem_base {
    ...
    public function item_parent_content_format_validation($parentcontent) {
        $format = SURVEYFIELD_AGE_FORMAT; // '[yy/mm]'

        $matches = $this->item_atomize_parent_content($parentcontent);

        if (!empty($matches)) {
            if ($matches[2] > 12) {
                return (get_string('err_exceedingmonth', 'surveyfield_age'));
            }
        } else {
    ...
    public function item_parent_content_content_validation($parentcontent) {
        ...
        if (!empty($matches)) {
            if ($matches[2] > 12) {
                throw new moodle_exception('Unexpected invalid month for age item: id: '.$this->itemid.', type '.$this->type.', plugin: '.$this->plugin);
            }
            if ($matches[1] < $this->lowerbound) {
                return (get_string('parentcontent_lowerthanminimum', 'surveyfield_age'));
            }
            if ($matches[1] > $this->upperbound) {
                return (get_string('parentcontent_greaterthanmaximum', 'surveyfield_age'));
            }
        } else {
            throw new moodle_exception('Unexpected invalid format for age item: id: '.$this->itemid.', type '.$this->type.', plugin: '.$this->plugin);
        }
    }
    ...
    public function item_parent_content_encode_value($parentcontent) {
        ...
        return $this->item_age_to_unix_time($matches[1], $matches[2]);
    }
    ...
    public function userform_child_is_allowed_dynamic($child_parentcontent, $data) {
        ...
        $status = true;
        $status = $status && ($data[$fieldname.'_year'] == $matches[1]);
