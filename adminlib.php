...
/**
 * This file contains the classes for the admin settings of the survey module.
 *
 * @package   mod_survey
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class survey_admin_page_manage_survey_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/survey/adminmanageplugins.php', array('subtype' => $subtype));
        parent::__construct('manage'.$subtype.'plugins',
                            get_string('manage'.$subtype.'plugins', 'survey'),
                            $url);
    }
...
class survey_plugin_manager {
     ...
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/survey/adminmanageplugins.php', array('subtype' => $subtype));
        $this->subtype = $subtype;
    }
    ...
    public function delete_plugin($plugin) {
        ...
        if ($confirm) {
            ...
            $shortsubtype = substr($this->subtype, strlen('survey'));
            $installxml = $CFG->dirroot.'/mod/survey/'.$shortsubtype.'/'.$plugin.'/db/install.xml';
     ...
     * @return None
     */
    public static function add_admin_survey_plugin_settings($subtype,
        ...
        foreach ($pluginsbyname as $pluginname => $plugin) {
            ...
            if ($admin->fulltree) {
                $shortsubtype = substr($subtype, strlen('survey'));
                include($CFG->dirroot."/mod/survey/$shortsubtype/$plugin/settings.php");
            }
